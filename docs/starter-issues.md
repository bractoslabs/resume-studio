...
Proposed work: Add E2E or unit coverage for dialog labels, Escape behavior, and focus behavior.
...
## 7. Add E2E Test for Backup and Restore

Type/labels: `tests`, `local-first`, `help wanted`

Context: Backup and restore are important for local-first storage.

Proposed work: Add a Playwright test that downloads a backup, clears local data, restores it, and confirms the resume returns.

Acceptance criteria:

- Test uses fictional data only.
- Restore behavior is clear in assertions.
- Test is reliable in CI.
