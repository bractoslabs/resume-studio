...
export const JobsPage = ({
  ...
  const jobTargets = resumes.flatMap((resume) =>
    (resume.jobTargets ?? []).map((job) => ({ ...job, resumeId: resume.id, resumeTitle: resume.title })),
  );
  const applications = resumes.flatMap((resume) =>
    (resume.applications ?? []).map((app) => ({ ...app, resumeId: resume.id, resumeTitle: resume.title })),
  );
  const addApplication = () => {
    if (!activeResume) return;
    updateResume(activeResume.id, {
      applications: [
        { id: uid("app"), company: "", role: activeResume.targetRole ?? "", status: "interested" },
    ...
    return (
      <div className="content-page">
        <header className="page-header">
          <div>
            <h1>Job Targets</h1>
            <p>Track job targets and applications once you have a resume to work from.</p>
          </div>
  ...
  return (
    <div className="content-page">
      <header className="page-header">
        <div>
          <h1>Job Targets</h1>
          <p>Global workspace for job targets and applications across all resumes.</p>
        </div>
        <div className="page-actions">
          <Button className="primary" onClick={addApplication} disabled={!activeResume}>
      ...
      <section className="jobs-table">
        ...
        {jobTargets.length === 0 ? (
          <div className="empty-state">
            <h2>No job targets yet</h2>
            <p>Use Keyword & Fit Check from a resume to save a target role.</p>
          </div>
        ) : (
          (jobTargets as JobTargetRow[]).map((job) => (
            <article className="job-row" key={job.id}>
              <div>
                <strong>{job.title}</strong>
                <p>
                  {job.company || "Company not set"} · {job.resumeTitle}
                </p>
                <p className="muted">Next action: {nextActionForJob(job)}</p>
              </div>
              <label>
                Status
                <select
                  ...
                  onChange={(event) =>
                    updateResume(job.resumeId, (resume) => ({
                      jobTargets: resume.jobTargets.map((item) =>
                        item.id === job.id ? { ...item, status: event.target.value as JobTarget["status"] } : item,
                      ),
                    }))
                  }
                >
              ...
              <label className="span-2">
                Notes
                <textarea
                  value={job.checklist?.join("\n") ?? ""}
                  onChange={(event) =>
                    updateResume(job.resumeId, (resume) => ({
                      jobTargets: resume.jobTargets.map((item) =>
                        item.id === job.id ? { ...item, checklist: event.target.value.split("\n").filter(Boolean) } : item,
                      ),
                    }))
                  }
                />
      ...
      <section className="jobs-table">
        ...
        {applications.length === 0 ? (
          <div className="empty-state">
            ...
            <p>Add a job when you apply or save a role you want to tailor for.</p>
          </div>
        ) : (
          (applications as ApplicationRow[]).map((app) => {
            ...
            return (
              <article className="job-row" key={app.id}>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      updateResume(resume.id, {
                        applications: resume.applications.map((item) =>
                          item.id === app.id ? { ...item, company: event.target.value } : item,
                        ),
                      })
                    }
                  />
                ...
                <label>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      updateResume(resume.id, {
                        applications: resume.applications.map((item) =>
                          item.id === app.id ? { ...item, role: event.target.value } : item,
                        ),
                      })
                    }
                  />
                ...
                <label>
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      updateResume(resume.id, {
                        applications: resume.applications.map((item) =>
                          item.id === app.id ? { ...item, status: event.target.value as ApplicationRecord["status"] } : item,
                        ),
                      })
                    }
                  >
                ...
                <label>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      updateResume(resume.id, {
                        applications: resume.applications.map((item) =>
                          item.id === app.id ? { ...item, jobLink: event.target.value } : item,
                        ),
                      })
                    }
                  />
                ...
                <label>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      updateResume(resume.id, {
                        applications: resume.applications.map((item) =>
                          item.id === app.id ? { ...item, dateApplied: event.target.value } : item,
                        ),
                      })
                    }
                  />
                ...
                <label>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      updateResume(resume.id, {
                        applications: resume.applications.map((item) =>
                          item.id === app.id ? { ...item, followUpDate: event.target.value } : item,
                        ),
                      })
                    }
                  />
                ...
                <p className="muted">
                  Resume: {app.resumeTitle} · Next action: {nextActionForJob(app)}
                </p>
                <label className="span-2">
                  Notes
                  <textarea
                    ...
                    onChange={(event) =>
                      updateResume(resume.id, {
                        applications: resume.applications.map((item) =>
                          item.id === app.id ? { ...item, notes: event.target.value } : item,
                        ),
                      })
                    }
                  />
