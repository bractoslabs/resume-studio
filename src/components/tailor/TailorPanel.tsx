...
export const TailorPanel = ({ value, setValue, analyzed, setAnalyzed, report, onCreateVersion, onSaveTarget }: TailorPanelProps) => {
  ...
  const missingSuggestions = (report?.suggestions ?? []).filter(
    (suggestion) => termReview[suggestion.keyword] !== "do-not-have" && termReview[suggestion.keyword] !== "not-relevant",
  );
    ...
    <div className="workflow-panel tailor-panel">
      <div className="panel-heading">
        ...
        <div className="inline-actions">
          <Button className="primary" disabled={!value.trim()} onClick={() => setAnalyzed(true)}>
          ...
          <Button disabled={!value.trim()} onClick={() => onSaveTarget(termReview)}>
            Save job target
          </Button>
        </div>
      </div>
      <textarea
        className="job-input refined-job-input"
        value={value}
        onChange={(event) => {
      ...
      {analyzed && report && (
        <div className="tailor-results">
          <section className="job-summary-card">
            ...
            <h3>{report.title}</h3>
            <p>
              <strong>{report.overlapLabel ?? "Keyword overlap"}</strong>. This measures keyword overlap only. It is not a qualification
              score or a guarantee of ATS results.
            </p>
            <div className="score-row">
              <span>
                <strong>{report.coverage?.requiredSkills ?? 0}%</strong>Required coverage
              </span>
              <span>
                <strong>{report.coverage?.tools ?? 0}%</strong>Tools coverage
              </span>
              <span>
                <strong>{report.coverage?.domain ?? 0}%</strong>Domain language
              </span>
              <span>
                <strong>{report.coverage?.seniority ?? 0}%</strong>Seniority signals
              </span>
            </div>
            <div className="inline-actions">
              <Button className="primary" onClick={() => onCreateVersion(termReview)}>
                Create job-specific copy
              </Button>
          ...
          {visibleTerms.length === 0 ? (
            <section className="empty-state">
              <h3>Few confident job signals found</h3>
              <p>
                We could not confidently extract many job requirements. Try pasting the full job description, including responsibilities and
                qualifications.
              </p>
            </section>
          ) : (
            <>
              ...
              <section className="alignment-grid">
                ...
                <KeywordList
                  title="Missing from resume: add only if true"
                  values={report.missing.filter((term: string) => termReview[term] !== "do-not-have")}
                />
          ...
          <section className="suggestion-list">
            ...
            {missingSuggestions.length === 0 ? (
              <p className="muted">
                Your resume already covers the main extracted keywords, or missing terms were marked not relevant/not true. Review wording
                and export an ATS-safe version.
              </p>
            ) : (
              missingSuggestions.map((suggestion) => (
                <article key={suggestion.keyword}>
                  ...
                  <p>{suggestion.note}</p>
                  <p>
                    <strong>Suggested place:</strong> {suggestion.section}
                  </p>
                  <p>
                    <strong>Truth check:</strong> {suggestion.verificationPrompt}
                  </p>
