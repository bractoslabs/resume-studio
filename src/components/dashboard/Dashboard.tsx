...
export const Dashboard = ({
  ...
  <div className="dashboard-page">
    ...
    <section className="local-save-strip" aria-label="Local save and backup status">
      <div>
        <p><Save size={15} /> {lastBackupAt ? `Saved locally · Last backup: ${formatDate(lastBackupAt)} at ${timeOnly(lastBackupAt)}` : backupRecommended ? "Saved locally · Backup recommended" : "Saved locally"}</p>
      ...
      <div className="local-save-actions">
        <Button onClick={() => downloadBackup()} disabled={!hasResumes}><Download size={15} /> Download backup</Button>
        <label className="btn quiet-btn"><Upload size={15} /> Restore<input type="file" accept="application/json" hidden onChange={async (event) => {
    ...
    {hasResumes && <div className="dashboard-tools">
      <label className="search"><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search title, role, or tags" /></label>
              ...
              <div className="resume-card-head">
                <div>
                  <div className="resume-title-row">
                    <h2>{resume.title}</h2>
                    <button className="icon-btn subtle-icon-btn" onClick={() => renameResume(resume)} aria-label={`Rename ${resume.title}`} title="Rename resume"><Pencil size={15} /></button>
                  </div>
                  <p>{resume.targetRole || "No target role set"}</p>
                </div>
                <span className={`score-chip ${scoreTone(report.scores.overall)}`}>{done}/{progress.length} reviewed</span>
              </div>
              <div className="card-meta">
                <span>Last edited {formatDate(resume.updatedAt)}</span>
                <span>{getTemplate(resume.templateId).name}</span>
                {resume.lastExportedAt && <span>Last exported {formatDate(resume.lastExportedAt)}{resume.lastExportedFormat ? ` (${resume.lastExportedFormat})` : ""}</span>}
                <span>{pageCountEstimate(plain)} page est.</span>
              </div>
              <div className="mini-progress"><span style={{ width: `${Math.round((done / progress.length) * 100)}%` }} /><strong>{done}/{progress.length} checklist</strong></div>
              <div className="tags">{resume.tags.map((tag) => <span key={tag}>{tag}</span>)}</div>
              <div className="card-actions resume-card-actions">
                <Button className="primary" onClick={() => selectResume(resume.id)}>Open</Button>
                <Button onClick={() => reviewResume(resume.id)}><FileCheck2 size={16} /> Review</Button>
                <Button onClick={() => exportMarkdown(resume)}><FileDown size={16} /> Export</Button>
                <details className="resume-more-menu">
                  <summary aria-label={`More actions for ${resume.title}`} title="More actions"><MoreHorizontal size={18} /></summary>
                  <div>
                    <button onClick={() => renameResume(resume)}><Pencil size={15} /> Rename</button>
                    <button onClick={() => duplicateResume(resume)}><Copy size={15} /> Duplicate</button>
                    <button onClick={() => exportMarkdown(resume)}><FileDown size={15} /> Export copy</button>
                    <button className="danger" onClick={() => deleteResume(resume)}><Trash2 size={15} /> Delete</button>
                  </div>
                </details>
              </div>
            </article>
