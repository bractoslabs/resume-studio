...
const modeIssueFilter = (mode: ReviewMode, issue: ResumeReviewIssue) => {
  ...
  if (mode === "ats") return ["ats-readability", "formatting-risk", "keyword-match", "contact", "structure", "date-consistency", "export-safety"].includes(issue.category);
...
export const ReviewPanel = ({ review, issues, markdown, setMarkdown, createSystemSnapshot, editSection, ignoreIssue, onReviewed }: ReviewPanelProps) => {
  ...
  const visibleScores = scores.filter((score) => {
    ...
    if (mode === "ats") return ["Contact Info", "Structure", "ATS Readability", "Formatting Risk", "Keyword Match", "Export Safety"].includes(score.label);
    return ["Bullet Quality", "Impact / Metrics", "Recruiter Skim", "Structure"].includes(score.label);
  });
  const verdict = review.plainText.text.trim().length < 220
  ...
  return (
    <div className="workflow-panel review-panel">
      <div className="segmented review-mode-tabs" role="tablist" aria-label="Review mode">
        {modeLabels.map(([key, label]) => <button key={key} className={mode === key ? "active" : ""} onClick={() => setMode(key)}>{label}</button>)}
      </div>
      <div className="review-hero">
        ...
        <details className="score-details">
          ...
          <div className="score-row">
            {scores.map((score) => <span key={score.label}><strong>{score.value}</strong>{score.label}</span>)}
          </div>
          <ul className="review-explainers">
            {review.scoringExplanation.map((item: string) => <li key={item}>{item}</li>)}
          </ul>
        </details>
      </div>
      <section className="intelligence-grid">
        {visibleScores.map((score) => <article key={score.label}><strong>{score.value}</strong><span>{score.label}</span><p>{score.explanation}</p></article>)}
      </section>
      ...
      <div className="inline-actions"><Button onClick={() => setShowAll(!showAll)}>{showAll ? "Show fewer issues" : "Show all issues"}</Button></div>
      {groups.map(([title, list, defaultCount]) => {
        ...
        if (!showAll && defaultCount === 0 && list.length > 0) {
          return (
            <details className="issue-group" key={title}>
              <summary><h3>{title}</h3><span>{list.length} collapsed</span></summary>
              {list.map((issue) => <IssueCard key={issue.id} issue={issue} markdown={markdown} setMarkdown={setMarkdown} createSystemSnapshot={createSystemSnapshot} editSection={editSection} ignoreIssue={ignoreIssue} />)}
            </details>
          );
        }
        return (
        <section className="issue-group" key={title}>
          <h3>{title}</h3>
          {list.length === 0 ? <p className="muted">Nothing in this group right now.</p> : visible.map((issue) => <IssueCard key={issue.id} issue={issue} markdown={markdown} setMarkdown={setMarkdown} createSystemSnapshot={createSystemSnapshot} editSection={editSection} ignoreIssue={ignoreIssue} />)}
        </section>
        );
      })}
    </div>
...
const SectionReviewGrid = ({ sections, editSection }: { sections: ResumeSectionReview[]; editSection: (section: string) => void }) => (
  <section className="section-review-grid">
    <header><h3>Section review</h3><p>Each section is checked for presence, clarity, issue density, and fit with common resume parsing expectations.</p></header>
    <div>
...
const AchievementAudit = ({ audit }: { audit: ResumeAchievementAudit }) => {
  ...
  return (
    <section className="achievement-audit">
      <header><h3>Key achievements audit</h3><p>Strong bullets usually combine action, scope, metric, and outcome without inventing facts.</p></header>
      <div className="achievement-metrics">
        <span><strong>{audit.completeBullets}/{audit.totalBullets}</strong> complete bullets</span>
        <span><strong>{pct(audit.metricBullets)}%</strong> include metrics</span>
        <span><strong>{pct(audit.scopeBullets)}%</strong> include scope</span>
        <span><strong>{pct(audit.outcomeBullets)}%</strong> include outcomes</span>
      </div>
      <div className="role-audit-list">
        {audit.roleSummaries.slice(0, 5).map((role) => (
          <article key={role.section}>
            <strong>{role.section}</strong>
            <p>{role.completeCount}/{role.bulletCount} bullets have the full achievement pattern. {role.recommendation}</p>
          </article>
        ))}
      </div>
...
const AtsOptimizationPanel = ({ review }: { review: ResumeReviewResult }) => (
  <section className="ats-optimization-panel">
    ...
    <details className="plain-text-preview">
      ...
      <p><strong>Parse confidence:</strong> {review.plainText.parseConfidence}%</p>
      {review.plainText.warnings.length > 0 && <ul>{review.plainText.warnings.map((warning: string) => <li key={warning}>{warning}</li>)}</ul>}
      {review.plainText.readingOrderNotes.length > 0 && <ul>{review.plainText.readingOrderNotes.map((note: string) => <li key={note}>{note}</li>)}</ul>}
...
const KeywordPills = ({ title, values }: { title: string; values: string[] }) => (
  <section className="keyword-pill-group">
    <h4>{title}</h4>
    {values.length ? <div>{values.map((value) => <span key={value}>{value}</span>)}</div> : <p className="muted">None detected.</p>}
  </section>
);
...
const IssueCard = ({ issue, markdown, setMarkdown, createSystemSnapshot, editSection, ignoreIssue }: IssueCardProps) => (
  <article className={`issue-card severity-${issue.severity}`} key={issue.id}>
    <div>
      ...
      <h4>{issue.title}</h4>
      <p><strong>Where:</strong> {issue.location}</p>
      <p><strong>Why it matters:</strong> {issue.whyItMatters}</p>
      <p><strong>Suggested fix:</strong> {issue.suggestedFix}</p>
      <p className="muted">{issue.safeAutoFix ? "Safe mechanical fix available. A snapshot is saved before it runs." : "Requires user judgment or verified facts."}</p>
    </div>
    <div className="issue-actions">
      {issue.fix ? <Button className="primary" onClick={() => {
      ...
      }}>{issue.fix.label}</Button> : <Button onClick={() => editSection(issue.location)}>Edit section</Button>}
...
const BulletImprover = ({ markdown, setMarkdown }: { markdown: string; setMarkdown: (markdown: string) => void }) => {
  ...
  const bullets = experience.split("\n").map((line) => line.trim()).filter((line) => /^[-*]\s+/.test(line)).slice(0, 4);
  if (!bullets.length) return <section className="bullet-lab compact-lab"><h3>Bullet improvement lab</h3><p>No experience bullets found yet.</p></section>;
  return (
