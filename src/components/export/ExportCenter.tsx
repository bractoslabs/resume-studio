    ...
    <div className="workflow-panel export-panel">
      ...
      {preExportIssues.length > 0 && (
        <section className="pre-export-warning" aria-label="Pre-export warnings">
          ...
          <ul>
            {preExportIssues.slice(0, 4).map((issue) => (
              <li key={issue.id}>
      ...
      <section className="format-list">
        ...
        {(
          [
            ...
            [
              "Resume Studio JSON",
              "Complete portable resume file with Markdown, settings, versions, jobs, and private notes.",
              () => exportResumeForgeJson(resume),
              true,
            ],
          ...
          ] satisfies ExportAction[]
        ).map(([label, description, action, enabled]) => (
          <article key={label}>
            <div>
              <strong>{label}</strong>
              <p>{description}</p>
            </div>
            <Button disabled={!enabled} onClick={() => runExport(label, action)}>
              {enabled ? "Export" : "Unavailable"}
            </Button>
          </article>
        ))}
      </section>
      <section className="copy-actions">
        <h3>Copy</h3>
        <p className="export-backup-note">
          Before sending this resume out, consider downloading a backup. Your work is saved locally in this browser only.
        </p>
        <Button onClick={() => runExport("Markdown copied", () => writeClipboard(resume.markdown))}>
          <Copy size={16} /> Copy Markdown
        </Button>
        <Button onClick={() => runExport("Plain text copied", () => writeClipboard(renderMarkdown(resume.markdown, true).plainText))}>
          <Clipboard size={16} /> Copy plain text for forms
        </Button>
        <Button onClick={() => runExport("Backup", () => downloadBackup())}>
          <Archive size={16} /> Download backup
        </Button>
      </section>
    </div>
  );
...
const FitAssistant = ({
  ...
  return (
    <section className={`fit-assistant ${shouldAssist ? "needs-fit" : ""}`}>
      <div>
        <h3>
          <Minimize2 size={16} /> Fit assistant
        </h3>
        <p>
          {shouldAssist
            ? "This resume may benefit from fitting before export."
            : "This resume looks reasonably balanced for the current page target."}
        </p>
      </div>
      <div className="fit-metrics">
        <span>
          <strong>{activePageCount}</strong> pages
        </span>
        <span>
          <strong>{wordCount}</strong> words
        </span>
        <span>
          <strong>{bullets.length}</strong> bullets
        </span>
        <span>
          <strong>{longBullets.length}</strong> long bullets
        </span>
      </div>
      <div className="fit-actions">
        <Button onClick={() => runExport("Compact spacing", () => applyDesignPatch(compactPatch), false)}>Apply compact spacing</Button>
        <Button disabled={!longBullets.length} onClick={() => runExport("Trim markers", trimLongestBullets, false)}>
          Mark longest bullets
        </Button>
        <Button onClick={() => runExport("ATS-safe mode", () => applyDesignPatch({ template: "ats-classic" }), false)}>
          Use ATS Classic
        </Button>
      </div>
      <p className="muted">
        Fit changes are conservative. Review marked bullets before exporting; never remove verified impact just to hit one page.
      </p>
    </section>
  );
};
