        ...
        <div className="feedback-actions">
          {feedbackDestinations.map((destination) => (
            <a
              ...
              className={`btn ${destination.primary ? "primary" : ""}`}
          ...
          <a
            className="btn"
            href="mailto:labs@bractos.com?subject=Resume%20Studio%20Feedback"
            onClick={() => setToast("Feedback email opened")}
          >
            Email Bractos Labs
          </a>
        </div>
        <p className="feedback-privacy-note">
          Please do not post private resume content, personal contact information, job application details, or other sensitive data in a
          public GitHub issue.
        </p>
        ...
        <div className="import-grid">
          <section className="import-choice-card upload-card">
            ...
            <label className="btn import-upload-btn">
              ...
              <input
                type="file"
                accept=".doc,.docx,.pdf,.md,.markdown,.txt,.json,.yaml,.yml,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document"
                hidden
        ...
        {draft && (
          <section className="import-review">
            ...
            <div className="import-review-grid">
              <section className="import-review-panel">
                ...
                <dl className="import-review-details">
                  <div>
                    <dt>Name</dt>
                    <dd>{draft.review.contact.name || "Not detected"}</dd>
                  </div>
                  <div>
                    <dt>Title</dt>
                    <dd>{draft.review.contact.title || "Not detected"}</dd>
                  </div>
                  <div>
                    <dt>Email</dt>
                    <dd>{draft.review.contact.email || "Not detected"}</dd>
                  </div>
                  <div>
                    <dt>Phone</dt>
                    <dd>{draft.review.contact.phone || "Not detected"}</dd>
                  </div>
                  <div>
                    <dt>Location</dt>
                    <dd>{draft.review.contact.location || "Not detected"}</dd>
                  </div>
                </dl>
              </section>
              <section className="import-review-panel">
                ...
                <dl className="import-review-details">
                  <div>
                    <dt>Sections</dt>
                    <dd>{draft.sections.length ? draft.sections.join(", ") : "None"}</dd>
                  </div>
                  <div>
                    <dt>Bullets</dt>
                    <dd>{draft.review.bulletCount}</dd>
                  </div>
                  <div>
                    <dt>Confidence</dt>
            ...
            <div className="import-review-grid">
              <section className="import-review-panel">
                ...
                {draft.review.repairedFields.length ? (
                  <ul className="import-review-list">
                    {draft.review.repairedFields.map((field) => (
              ...
              <section className="import-review-panel">
                ...
                {draft.review.ignoredFields.length ? (
                  <ul className="import-review-list">
                    {draft.review.ignoredFields.map((field) => (
                      <li key={field}>{field}</li>
                    ))}
                  </ul>
                ) : (
            ...
            <details>
              ...
              <textarea
                aria-label="Clean imported Markdown"
      ...
      <section className="modal-card new-resume-card">
        <header>
          <div>
            ...
            <p>
              {step === "start"
                ? "Choose the fastest way to begin."
                : "Add the basics now so Review and Export have a stronger starting point."}
            </p>
          </div>
          <button className="icon-btn" onClick={onClose} aria-label="Close new resume">
            <X size={18} />
          </button>
        </header>
        {step === "start" ? (
          ...
          <div className="setup-flow">
            ...
            <div className="setup-grid">
              <section className="setup-fields">
                <label>
                  ...
                  <input
                    autoFocus
                    value={setup.resumeTitle}
                    onChange={(event) => updateSetup({ resumeTitle: event.target.value })}
                ...
                <div className="form-grid two">
                  <label>
                    Your name
                    <input value={setup.name} onChange={(event) => updateSetup({ name: event.target.value })} placeholder="Jordan Lee" />
                  </label>
                  <label>
                    Target role
                    <input
                ...
                <label>
                  ...
                  <select
                    ...
                    onChange={(event) => updateSetup({ templateId: event.target.value as NewResumeSetup["templateId"] })}
                  >
              ...
              <aside className="setup-next-steps">
                ...
                <ol>
                  <li>
                    <strong>Complete the draft.</strong>
                    <span>{setup.startMode === "guided" ? "You will land in guided mode." : "You will land in Markdown mode."}</span>
                  </li>
                  <li>
            ...
            <div className="modal-actions">
              ...
              <Button className="primary" disabled={!canCreate} onClick={() => onTemplate(setup)}>
                <FileDown size={15} /> Create resume
              </Button>
            </div>
          </div>
        )}
...
export const RenameResumeDialog = ({
  draft,
  setDraft,
  onCancel,
  onSave,
  ...
  <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="Rename resume">
    <section className="modal-card confirm-card">
      ...
      <div className="inline-actions">
        <Button onClick={onCancel}>Cancel</Button>
        <Button className="primary" disabled={!draft.title.trim()} onClick={onSave}>
          Save name
        </Button>
      </div>
    </section>
...
export const SaveVersionDialog = ({
  ...
  setDraft: (draft: SaveVersionDraft | null) => void;
  onCancel: () => void;
  onSave: () => void;
}) => (
...
export const RestoreBackupDialog = ({
  ...
  draft: RestoreDraft;
  setDraft: (draft: RestoreDraft) => void;
  onCancel: () => void;
  onSafetyBackup: () => void;
  onRestore: () => void;
}) => (
  <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="Restore backup">
    <section className="modal-card confirm-card">
      ...
      <p>
        Review this backup file before restoring. Merge keeps the current workspace and adds restored resumes as copies. Replace removes the
        current resumes and app data saved in this browser, then loads the backup file.
      </p>
      <dl className="restore-preview">
        <div>
          <dt>Resumes</dt>
          <dd>{draft.preview.resumeCount}</dd>
        </div>
        <div>
          <dt>Jobs and applications</dt>
          <dd>{draft.preview.jobCount}</dd>
        </div>
        <div>
          <dt>Versions</dt>
          <dd>{draft.preview.versionCount}</dd>
        </div>
        <div>
          <dt>Backup date</dt>
          <dd>{draft.preview.exportedAt ? formatDate(draft.preview.exportedAt) : "Unknown"}</dd>
        </div>
      </dl>
      <fieldset className="restore-choice">
        <legend>Restore choice</legend>
        <label>
          <input type="radio" checked={draft.mode === "merge"} onChange={() => setDraft({ ...draft, mode: "merge" })} /> Merge with current
          workspace
        </label>
        <label>
          <input type="radio" checked={draft.mode === "replace"} onChange={() => setDraft({ ...draft, mode: "replace" })} /> Replace current
          workspace
        </label>
      </fieldset>
      {draft.mode === "merge" && (
        <p className="status-note">Merge keeps your current local data and adds the backup contents as restored copies.</p>
      )}
      {draft.mode === "replace" && (
        <p className="status-note error">
          Replace removes the current local workspace in this browser. Resume Studio does not have a cloud copy. Download the current
          workspace first if you may need it later.
        </p>
      )}
      <div className="inline-actions">
        <Button onClick={onCancel}>Cancel</Button>
        <Button onClick={onSafetyBackup}>
          <Download size={15} /> Download current workspace first
        </Button>
...
export const ClearLocalDataDialog = ({
  ...
  <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="Clear local data">
    <section className="modal-card confirm-card">
      ...
      <p>
        This will remove resumes and app data saved in this browser. Resume Studio does not have a cloud copy. Download a backup first if
